...
{
    unsigned int RECnt = 0;
    unsigned int CurVal = 0;
    char dataval[3] = { 0x00, 0x00, 0x00 };
    ...
    if (panelresult != 0)
    {
        ...
        {
            REData = malloc(ESize/2);

            while (([self isHexchar:EData[RECnt]]) && ([self isHexchar:EData[RECnt+1]]) && (RECnt < ESize))
            {
                dataval[0] = EData[RECnt];
                dataval[1] = EData[RECnt+1];
                
                dataout = [self hex2uchar:dataval];
                REData[CurVal] = dataout;
